...
def batch_all_reduce_grad(model: nn.Module):
  params_with_grad = [p for p in model.parameters() if p.grad is not None]
  if len(params_with_grad) > 0:
    # 1.将所有梯度展平为一个大向量
    grads = [p.grad for p in params_with_grad]
    flatten_gradients = torch.nn.utils.parameters_to_vector(grads)

    # 2.使用all-reduce进行同步
    dist.all_reduce(flatten_gradients, op=dist.ReduceOp.AVG, async_op=False)

    # 3.将同步后的展平梯度还原并赋值回参数
    torch.nn.utils.vector_to_parameters(flatten_gradients, grads)
...
def data_parallelism_main_with_benchmarking(
  ...
  # 使用自定义的DDP类包装model，完成参数同步以及注册梯度同步钩子
  ...
  for _ in range(num_warmups):
    logits = model(local_batch_x) # 前向传播
    loss = cross_entropy(logits, local_batch_y)
    optimizer.zero_grad(set_to_none=True)
    loss.backward() # 反向传播（触发梯度同步钩子）
    # 等待梯度同步完成
